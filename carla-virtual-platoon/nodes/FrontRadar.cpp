...
FrontRadarPublisher::FrontRadarPublisher(boost::shared_ptr<carla::client::Actor> actor)
    ...
    radar_sensor_tick = sanitize_numeric_attr(radar_sensor_tick);
    radar_horizontal_fov = sanitize_numeric_attr(radar_horizontal_fov);
    radar_vertical_fov = sanitize_numeric_attr(radar_vertical_fov);
    radar_range = sanitize_numeric_attr(radar_range);
    ...
    if (!radar_bp) {
        RCLCPP_ERROR(this->get_logger(), "[FrontRadarPublisher] Failed to find radar blueprint. Terminating process.");
        throw std::runtime_error("Radar blueprint not found");
    }
    ...
    if (!radar_actor) {
        RCLCPP_ERROR(this->get_logger(), "[FrontRadarPublisher] Failed to spawn radar sensor. Terminating process.");
        throw std::runtime_error("Radar spawn failed");
    }
    radar = boost::static_pointer_cast<cc::Sensor>(radar_actor);
    if (!radar) {
        RCLCPP_ERROR(this->get_logger(), "[FrontRadarPublisher] Failed to cast radar actor to sensor. Terminating process.");
        throw std::runtime_error("Radar cast failed");
    }
